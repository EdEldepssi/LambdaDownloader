...
def lambda_handler(event, context):
    ...
    baseline_file = '2023-12-31-10.json.gz'
    file_prefix = os.environ.get('FILE_PREFIX')
    while True:
        prev_file_name = get_prev_file_name(bucket_name, file_prefix, bookmark_file, baseline_file)
        print(f"Prev file name: {prev_file_name}")

        if prev_file_name is None:
            raise ValueError("Prev file name is None. This should not happen with a proper baseline file set.")
